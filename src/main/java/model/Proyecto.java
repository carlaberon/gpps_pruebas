...
public class Proyecto {
    ...
    public Proyecto(int id, String nombre, String descripcion, Boolean estado, String areaDeInteres,
        ...
        if (nombre.isEmpty()){
            throw new RuntimeException("Ups! El nombre ingresado no es válido.");
        }
        if(descripcion.isEmpty()){
            throw new RuntimeException("Ups! la descripción ingresada no es válida");

        }
        if(areaDeInteres.isEmpty()){
            throw new RuntimeException("Ups! el área de interés ingresada no es válida");

        }
    //falta validar los otros campos:   Estudiante, Director, Tutor
        this.id_proyecto = id;
        this.nombre = nombre;
